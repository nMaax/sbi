...
class MaskedBasePotential(metaclass=ABCMeta):
    def __init__(
        self,
...
class CustomMaskedPotential(Protocol):
    """Protocol for custom potential functions."""

    def __call__(
        self, input: Tensor, condition_mask: Tensor, edge_mask: Tensor, x_o: Tensor
    ) -> Tensor:
...
class CustomMaskedPotentialWrapper(MaskedBasePotential):
    """If `potential_fn` is a callable it gets wrapped as this."""

    def __init__(
        self,
        potential_fn: CustomMaskedPotential,  # type: ignore
