...
from sbi.inference.potentials.base_potential import BasePotential, MaskedBasePotential
from sbi.inference.potentials.score_fn_iid import get_iid_method
from sbi.neural_nets.estimators import ConditionalVectorFieldEstimator
from sbi.neural_nets.estimators.base import MaskedConditionalVectorFieldEstimator
...
def masked_vector_field_estimator_based_potential(
    vector_field_estimator: MaskedConditionalVectorFieldEstimator,
    prior: Optional[Distribution],
    x_o: Optional[Tensor],
    enable_transform: bool = True,
    **kwargs,
) -> Tuple["MaskedVectorFieldBasedPotential", TorchTransform]:
    ...
    potential_fn = MaskedVectorFieldBasedPotential(
        vector_field_estimator, prior, x_o, device=device, **kwargs
    )

    if prior is not None:
        latent_transform = mcmc_transform(
            prior, device=device, enable_transform=enable_transform
        )
    else:
        latent_transform = torch.distributions.transforms.identity_transform

    return potential_fn, latent_transform
...
class MaskedVectorFieldBasedPotential(MaskedBasePotential):
    def __init__(
        self,
        vector_field_estimator: MaskedConditionalVectorFieldEstimator,
        ...
        r"""
        Potential class for masked vector field estimators. Implements the potential
        function via the probability flow ODE and the gradient via the score estimator.
        If the vector field estimator does not define the score (SCORE_DEFINED = False),
...
class CallableDifferentiablePotentialFunction:
    ...
    def __init__(
        self,
        vector_field_based_potential: Union[
            MaskedVectorFieldBasedPotential, VectorFieldBasedPotential
        ],
    ):
