...
class MaskedNeuralODEFunc(Protocol):
    """Protocol for the function to be integrated by the MaskedNeuralODE."""

    def __call__(self, input: Tensor, times: Tensor) -> Tensor:
        r"""
        Callable that takes in the current state and time and returns the
        derivative of the state, given that the conditioning (masks) are
        handled internally by the wrapped estimator.

        Args:
            input: The current state (e.g., the full inputs tensor at time t).
            times: The time t.
...
class MaskedNeuralODE(LazyDistribution):
    r"""
    ...
    def forward(self, condition: Tensor, **kwargs) -> Distribution:
        ...
        return self.get_distribution(**ode_kwargs)

    @abstractmethod
    def get_distribution(self, **kwargs) -> Distribution:
        """
