...
class MaskedZukoNeuralODE(MaskedNeuralODE):
    def __init__(
        self,
    ...
    def get_distribution(self, **kwargs) -> Distribution:
        ...
        transform = FreeFormJacobianTransform(
            f=lambda t, input: self.f(input, t),
            t0=self.mean_base.new_tensor(self.t_min),
            t1=self.mean_base.new_tensor(self.t_max),
        ...
        return NormalizingFlow(
            transform=transform,
            # ! There was an .expand(condition.shape[:-1]) here before
            # ? Are shapes still correct?
            base=DiagNormal(self.mean_base, self.std_base),
        )
